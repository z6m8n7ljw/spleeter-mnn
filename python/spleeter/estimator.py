...
class Estimator(nn.Module):
    ...
    def inverse_stft(self, stft):
        ...
        wav = torch.istft(stft_complex, self.win_length, hop_length=self.hop_length, center=True, 
                          window=self.win)
        return wav.detach()
    ...
    def separate(self, wav):
        ...
        for net in self.instruments:
            mask = net(stft_mag)
            # print("\033[32m" + f'Type: {type(mask)}' + "\033[0m ", "\033[33m" + f'Size: {mask.shape}' + "\033[0m\n")
            masks.append(mask)
